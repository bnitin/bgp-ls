...
import org.topology.bgp_ls.net.ASType;
import org.topology.bgp_ls.net.attributes.MultiProtocolNLRI;
import org.topology.bgp_ls.net.attributes.NextHopPathAttribute;
...
import org.topology.bgp_ls.net.capabilities.Capability;
import org.topology.bgp_ls.netty.BGPv4Constants;
import org.topology.bgp_ls.netty.FSMState;
import org.topology.bgp_ls.netty.PeerConnectionInformation;
import org.topology.bgp_ls.netty.handlers.BgpEvent;
import org.topology.bgp_ls.netty.handlers.NotificationEvent;
import org.topology.bgp_ls.netty.protocol.BGPv4Packet;
import org.topology.bgp_ls.netty.protocol.FiniteStateMachineErrorNotificationPacket;
import org.topology.bgp_ls.netty.protocol.HoldTimerExpiredNotificationPacket;
import org.topology.bgp_ls.netty.protocol.KeepalivePacket;
import org.topology.bgp_ls.netty.protocol.NotificationPacket;
import org.topology.bgp_ls.netty.protocol.UnspecifiedCeaseNotificationPacket;
import org.topology.bgp_ls.netty.protocol.open.CapabilityListUnsupportedCapabilityNotificationPacket;
import org.topology.bgp_ls.netty.protocol.open.OpenNotificationPacket;
import org.topology.bgp_ls.netty.protocol.open.OpenPacket;
import org.topology.bgp_ls.netty.protocol.open.UnsupportedVersionNumberNotificationPacket;
import org.topology.bgp_ls.netty.protocol.update.UpdateNotificationPacket;
import org.topology.bgp_ls.netty.protocol.update.UpdatePacket;
import org.topology.bgp_ls.netty.service.BGPv4Client;
...
public class BGPv4FSM {
	...
	private void processRemoteUpdate(UpdatePacket message) {
		Set<MultiProtocolNLRI> mp = message.lookupPathAttributes(MultiProtocolNLRI.class);
		Set<PathAttribute> otherAttributes = message.filterPathAttributes(MultiProtocolNLRI.class, NextHopPathAttribute.class);
		Set<PathAttribute> pathAttributes = (Set<PathAttribute>)message.filterPathAttributes(PathAttribute.class);

		if(mp.size() > 0) {
			processRemoteUpdateMultiProtocol(mp, pathAttributes);
			processRemoteUp(mp, otherAttributes);
		}
	}

	private void processRemoteUp(Set<MultiProtocolNLRI> npNlri, Set<PathAttribute> attrs) {
		// TODO Auto-generated method stub	
	}

	private void processRemoteUpdateMultiProtocol(Set<MultiProtocolNLRI> mpNlri, Set<PathAttribute> attrs) {
		for(MultiProtocolNLRI mp : mpNlri) {
			try {
				MultiProtocolNLRIExport.export(mp, attrs);
			}  catch(Exception e) {
				log.error("Failed to export nlri " + mp.toString());
			}
		}
	}
